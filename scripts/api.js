...
app.post('/api/mint-nft', async (req, res) => {
  try {
    ...
    if (!recipient || !tokenURI) {
      return res
        .status(400)
        .json({ success: false, error: 'Recipient and tokenURI are required.' })
    }
    ...
    res.json({
      ...
      transaction: {
        ...
        from: tronWeb.address.fromHex(
          txInfo.raw_data.contract[0].parameter.value.owner_address
        ),
        to: tronWeb.address.fromHex(
          txInfo.raw_data.contract[0].parameter.value.contract_address
        ),
